    ...
    module ActiveShipping
      class Base < ShippingCalculator
        ...
        def country_weight_error?(package)
          ...
          raise Spree::ShippingError, "#{I18n.t(:shipping_error, scope: :spree)}: The maximum per package weight for the selected service from the selected country is #{max_weight} ounces." unless valid_weight_for_package?(package, max_weight)
        end
        ...
        def retrieve_rates(origin, destination, shipment_packages)
          ...
          error = Spree::ShippingError.new("#{I18n.t(:shipping_error, scope: :spree)}: #{message}")
