    ...
    def convert_package_to_weights_array(product_with_product_packages)
      weights = product_with_product_packages.map do |content_item|
        ...
        if (item_weight > max_weight) && max_weight > 0
          raise Spree::ShippingError, "#{I18n.t(:shipping_error, scope: :spree)}: The maximum per package weight for the selected service from the selected country is #{max_weight} ounces."
        end
